...
import {
  IAnalysisPayNumsType, IAnalysisPercentType, IAnalysisSalesType,
  IAnalysisStateType,
  IAnalysisVisitsType,
...
class Analysis extends Component<IAnalysisProps, IAnalysisInitState> {
  ...
  onTabChange = (key: string) => {
    console.log(key);
    if (key !== 'tab1') {
      this.setState({ key, chartSaleAndVisitTitle: '访问量趋势', listSaleAndVisitTitle: '门店访问量排名' });
    } else {
      this.setState({ key, chartSaleAndVisitTitle: '销售趋势', listSaleAndVisitTitle: '门店销售额排名' });
    }
  };
  ...
  render() {
    ...
    const { key, chartSaleAndVisitTitle, listSaleAndVisitTitle, rangeDate, selectedDate } = this.state;
    console.log('loading', loading);
    console.log('percent', percent);
    console.log('sales', sales);
    console.log('moment())', `${moment().year()}-${moment().week()}-${moment().day()}`);
    ...
    const chartSaleAndVisit = (<Row>
      <Col span={18}>
        ...
        <Chart height={300} data={sales} scale={salesScale} forceFit padding="auto">
          ...
          <Geom type="interval" position="month*value" tooltip={['month*value', (month, value) => ({
            name: month,
            value,
          })]}/>
        </Chart></Col>
      <Col span={6}>
        ...
        <List
          ...
          renderItem={(item, index) => (
            <div>
              {(index < 7) ? (<List.Item key={item.id} extra={item.rank}>
                {index < 3 ? <span className={styles.sales} style={{
                  color: '#fff',
                  backgroundColor: '#314659',
                }}>{index + 1}</span> : <span className={styles.sales}>{index + 1}</span>}
                <List.Item.Meta
    ...
    const salesExtra = (<div>
        <a onClick={() => this.selectDate('day')}
           className={selectedDate === 'day' ? styles.currentDate : ''}> {'今日'}</a>
        <a onClick={() => this.selectDate('week')}
           className={selectedDate === 'week' ? styles.currentDate : ''}> {'本周'}</a>
        <a onClick={() => this.selectDate('month')}
           className={selectedDate === 'month' ? styles.currentDate : ''}> {'本月'}</a>
        <a onClick={() => this.selectDate('year')}
           className={selectedDate === 'year' ? styles.currentDate : ''}> {'全年'}</a>
    ...
    const onlineSearchExtraItem = (<Menu>
        <Menu.Item>
          操作一
        </Menu.Item>
        <Menu.Item>
      ...
      <div>
        <Row gutter={16} style={{ paddingTop: 20 }}>
          <Col span={6}>
            <Card>
              <Row>
                <Col span={22}><span style={{ color: 'rgba(0,0,0,.45)' }}>总销售额</span></Col>
                <Col span={2}><AntdTooltip title="指标说明"><Icon type="info-circle"/></AntdTooltip></Col>
              </Row>
              <span style={{ fontSize: 30 }}>¥ 126,560</span>
              <Row style={{ height: 46 }} type="flex" align="bottom">
                <Col span={12}>周同比&nbsp;12%<Icon type="caret-up" style={{ color: 'red' }}/></Col>
                <Col span={12}>周同比&nbsp;12%<Icon type="caret-down" style={{ color: 'green' }}/></Col>
              </Row>
          ...
          <Col span={6}>
            <Card>
              <Row>
                <Col span={22}><span style={{ color: 'rgba(0,0,0,.45)' }}>访问量</span></Col>
                <Col span={2}><AntdTooltip title="指标说明"><Icon type="info-circle"/></AntdTooltip></Col>
              </Row>
              ...
              <Row style={{ height: 46 }} type="flex" align="bottom">
                <Col span={24}>
                  <Chart height={40} data={visits} forceFit padding="auto">
                    ...
                    />
                    <Geom type="area" position="year*value" tooltip={['year*value', (year, value) => ({
                      name: year,
                      value,
                    })]}/>
                    <Geom type="line" position="year*value" tooltip={['year*value', (year, value) => ({
                      name: year,
                      value,
                    })]}/>
                  </Chart>
          ...
          <Col span={6}>
            <Card>
              ...
              <Row style={{ height: 46 }} type="flex" align="bottom">
                <Col span={24}>
                  <Chart height={40} data={payNumbers} scale={payNumbersScale} forceFit padding="auto">
                    <Tooltip
                      showTitle={false}
                      crosshairs={{
                        type: 'rect',
          ...
          <Col span={6}>
            <Card>
              <Row>
                <Col span={22}><span style={{ color: 'rgba(0,0,0,.45)' }}>运营活动效果</span></Col>
                <Col span={2}><AntdTooltip title="指标说明"><Icon type="info-circle"/></AntdTooltip></Col>
              </Row>
        ...
        <Row style={{ paddingTop: 20 }}>
          <Col span={24}>
            <Card tabBarExtraContent={salesExtra} tabList={tabList} activeTabKey={key}
        ...
        <Row gutter={16} style={{ paddingTop: 20 }}>
          <Col span={12}>
            <Card title="线上热门搜索" extra={onlineSearchExtra}>
              <Row>
                <Col span={12} style={{ paddingLeft: 34, paddingRight: 34, marginBottom: 24 }}>
                  <Row>
                    <Col span={16}><span style={{ color: 'rgba(0,0,0,.45)' }}>搜索用户数</span></Col>
                    <Col span={6}><AntdTooltip title="指标说明"><Icon type="info-circle"/></AntdTooltip></Col>
                  </Row>
                  <Row style={{ height: 32, marginTop: 8 }} type="flex" align="top">
                    <Col span={12}><span style={{ fontSize: 24 }}>12,321</span></Col>
                    <Col span={12}><span style={{ fontSize: 16 }}>17.1</span><Icon type="caret-up" style={{ color: 'red' }}/></Col>
                  </Row>
                </Col>
                <Col span={12} style={{ paddingLeft: 34, paddingRight: 34, marginBottom: 24 }}>
                  <Row>
                    <Col span={16}><span style={{ color: 'rgba(0,0,0,.45)' }}>人均搜索次数</span></Col>
                    <Col span={6}><AntdTooltip title="指标说明"><Icon type="info-circle"/></AntdTooltip></Col>
                  </Row>
                  <Row style={{ height: 32, marginTop: 8 }} type="flex" align="top">
                    <Col span={12}><span style={{ fontSize: 24 }}>2.7</span></Col>
                    <Col span={12}><span style={{ fontSize: 16 }}>26.2</span><Icon type="caret-down" style={{ color: 'green' }}/></Col>
                  </Row>
                </Col>
              </Row>
            </Card>
